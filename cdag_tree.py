...
def save_final_ldd(unique_k: np.ndarray, counts_k: np.ndarray, emp_probs: np.ndarray, s: int, queries_count: int, prefix: str, l2_emp: float, s_star: int, suffix: str = ''):
    ...
    # define theo_dist
    # Determine c from prefix
    c = 5 if 'dag5' in prefix.lower() else 3

    theo_data = get_theoretical_dist(s_star, c)
    theo_dist = theo_data['dist']  # Extract prob dict for L2
    expected_k_theo = theo_data['expected_k']
    expected_exp_k_theo = theo_data['expected_exp_k']

    theo_max_k = max(theo_dist.keys()) if theo_dist else 0


    # compute L2_emp_theo
    L2_emp_theo = compute_l2_norm(emp_dist, theo_dist)

    all_k = sorted(set(unique_k) | set(theo_dist.keys()))
    ...
    theo_p = [theo_dist.get(k, 0.0) for k in all_k]

    # Compute expected level difference and exponential ratio (empirical)
    ...
    plt.ylabel("Probability")
    plt.title(f"{prefix} Final Level Difference Distribution (s={s}, {queries_count} queries, s*={s_star}, c={c})")
    plt.legend()

    # Theoretical curve for s*
    k_theo = np.array(sorted(theo_dist.keys()))
    p_theo = np.array([theo_dist[kk] for kk in k_theo])
    plt.plot(k_theo, p_theo, color='red', label=f'Theoretical (s*={s_star}, c={c})', marker='o', linestyle='-')
    plt.legend()
    
    plt.xticks(all_k)
    try:
         plt.savefig(os.path.join(full_ldd_dir, f"{prefix}_final_ldd_plot_s{s}_q{queries_count}{suffix}_sstar{s_star}_c{c}.png"))
    finally:
        plt.close()

    # CSV and Excel
    #overall_max_k = max(max_k, theo_max_k)
    ...
    dist_df.to_excel(os.path.join(full_ldd_dir, f"{prefix}_final_ldd_dist_table_s{s}_q{queries_count}{suffix}_sstar{s_star}_c{c}.xlsx"), index=False)

    # Save arrays for plots
    #l2_results[-1] = all_k.tolist()           # list of levels
    #l2_results[-1 -1]['theoretical_p'] = theo_p           # theoretical probs
    ...
    l2_results.append({
        ...
        'expected_k_theo': expected_k_theo,
        'expected_exp_k_theo': expected_exp_k_theo,
        'suffix': suffix,  #To distinguish stable/ongoing/final in CSV
        's_star': s_star  # Added: Include s_star in results
    })
    with open(os.path.join(full_ldd_dir, f"{prefix}_l2_norms_s{s}_q{queries_count}{suffix}_sstar{s_star}.txt"), 'w') as f:
        f.write(f"L2 norm between last two empirical distributions: {l2_emp:.6f}\n")
        f.write(f"L2 norm between final empirical and theoretical for s*: {L2_emp_theo:.6f}\n")
        f.write(f"Expected level difference (emp): {expected_k_emp:.6f}\n")
        f.write(f"Expected exponential ratio (emp): {expected_exp_k_emp:.6f}\n")
        f.write(f"Expected level difference (theo): {expected_k_theo:.6f}\n")
        f.write(f"Expected exponential ratio (theo): {expected_exp_k_theo:.6f}\n")
...
if __name__ == "__main__":
    ...
    # Generate whisker plots for expected level difference
    l2_df = pd.DataFrame(l2_results)
    for dataset in ['gowalla_dag3', 'uniform_dag3', 'gowalla_dag5', 'uniform_dag5']:
        ds_df = l2_df[l2_df['dataset'] == dataset]
        query_ranges = sorted(ds_df['query_range'].unique())
        emp_values = {qr: [] for qr in query_ranges}
        theo_values = {qr: None for qr in query_ranges}
        for qr in query_ranges:
            qr_df = ds_df[ds_df['query_range'] == qr].sort_values('queries_count')
            emp_values[qr] = qr_df[qr_df['queries_count'] >= 5000].iloc[::10]['expected_k_emp'].tolist()
            #stable_df = qr_df[qr_df['L2_emp'] < EPSILON]
            #if not stable_df.empty:
                #stable_count = stable_df['queries_count'].min()
                #post_stable = qr_df[qr_df['queries_count'] >= stable_count]
                #post_stable = qr_df[qr_df['queries_count'] >= stable_count].iloc[::10]
                #emp_values[qr] = post_stable['expected_k_emp'].tolist()
            #else:
                #emp_values[qr] = qr_df['expected_k_emp'].tolist()
            if not qr_df.empty:
                theo_values[qr] = qr_df.iloc[0]['expected_k_theo']  # Theoretical is constant per range
        data = [emp_values[qr] for qr in query_ranges]
        plt.figure()
        plt.boxplot(data, labels=query_ranges, whis=[0, 100])
        for i, qr in enumerate(query_ranges):
            if theo_values[qr] is not None:
                plt.plot(i + 1, theo_values[qr], 'ro', label='Theoretical' if i == 0 else None)
        plt.xlabel("Query Ranges")
        plt.ylabel("Expected Level Difference")
        plt.title(f"{dataset.capitalize()} Dataset")
        plt.legend()
        plt.savefig(os.path.join(BASE_OUTPUT_DIR, f"{dataset}_expected_level_diff_whisker.png"))
        plt.close()

    # Generate whisker plots for expected exponential ratio 
    for dataset in ['gowalla_dag3', 'uniform_dag3' , 'gowalla_dag5' , 'uniform_dag5']:
        ds_df = l2_df[l2_df['dataset'] == dataset]
        query_ranges = sorted(ds_df['query_range'].unique())
        emp_values = {qr: [] for qr in query_ranges}
        ...
        for qr in query_ranges:
            qr_df = ds_df[ds_df['query_range'] == qr].sort_values('queries_count')
            emp_values[qr] = qr_df[qr_df['queries_count'] >= 5000].iloc[::10]['expected_exp_k_emp'].tolist()
            #stable_df = qr_df[qr_df['L2_emp'] < EPSILON]
            #if not stable_df.empty:
                #stable_count = stable_df['queries_count'].min()
                # post_stable = qr_df[qr_df['queries_count'] >= stable_count]
                #post_stable = qr_df[qr_df['queries_count'] >= stable_count].iloc[::10]
                #emp_values[qr] = post_stable['expected_exp_k_emp'].tolist()  # Changed key
            #else:
                #emp_values[qr] = qr_df['expected_exp_k_emp'].tolist()  # Changed key
            if not qr_df.empty:
                theo_values[qr] = qr_df.iloc[0]['expected_exp_k_theo']  # Changed key
        data = [emp_values[qr] for qr in query_ranges]
        plt.figure()
        plt.boxplot(data, labels=query_ranges, whis=[0, 100])
        for i, qr in enumerate(query_ranges):
        ...
        plt.title(f"{dataset.capitalize()} Dataset - Exponential Ratio")
        plt.legend()
        plt.savefig(os.path.join(BASE_OUTPUT_DIR, f"{dataset}_expected_exp_ratio_whisker.png"))  
        plt.close()
