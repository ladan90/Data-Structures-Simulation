...
final_table_rows = []   # <--- ADD THIS LINE (for the big table)

# ========== CONFIGURATION ==========
NYC_FILE = "yellow_tripdata_2024-01.parquet"
...
EPSILON = 0.001
N_FIXED = 1048576  # Fixed N=2^20
n_FIXED = 19  # Since 2^{n+1}=2^20
TIMESPAN = [0, 49626707]
...
BASE_OUTPUT_DIR = "DenseDAG4_NYC"

# ========== DATA PREPARATION ==========

def load_nyc_taxi_data(filename: str) -> List[int]:
    """
    Load NYC Yellow Taxi pickup times from Parquet,
    remove garbage timestamps outside January 2024,
    normalize to [0, ...] in seconds,
    return sorted unique normalized seconds, limited to first N_FIXED.
    """
    print(f"Attempting to load file: {filename}")
    print(f"Full absolute path: {os.path.abspath(filename)}")
    ...
    try:
        df = pd.read_parquet(filename, columns=["tpep_pickup_datetime"])
    except Exception as e:
        print(f"Error loading Parquet: {e}")
        return []

    print("Total rows in raw file:", len(df))
    ...
    # Convert to seconds since epoch (robust, no FutureWarning)
    ts_seconds = ts.astype("int64") // 1_000_000

    # === CRITICAL FILTER: remove garbage timestamps (2003, 2008, etc.) ===
    jan2024_start = 1704067200   # 2024-01-01 00:00:00 UTC
    jan2024_end   = 1706659200   # 2024-02-01 00:00:00 UTC

    valid = (ts_seconds >= jan2024_start) & (ts_seconds < jan2024_end)
    ts_seconds = ts_seconds[valid]

    print(f"Rows after Jan 2024 filter: {len(ts_seconds):,} "
          f"(removed {len(valid) - len(ts_seconds):,} outlier rows)")

    if len(ts_seconds) == 0:
        raise ValueError("No valid January 2024 data after filtering!")

    min_time = int(ts_seconds.min())
    print(f"Min timestamp AFTER cleaning (seconds): {min_time} "
          f"→ {pd.to_datetime(min_time, unit='s')}")

    # Normalize to start at 0
    times = sorted(set(int(t - min_time) for t in ts_seconds))

    print(f"Distinct timestamps after cleaning & before limiting: {len(times):,}")
    print(f"Real timespan: {times[-1] - times[0]:,} seconds "
          f"({(times[-1] - times[0]) / 86400:.1f} days)")

    if len(times) < N_FIXED:
        print(f"Warning: Only {len(times)} distinct times available, using all.")

    return times[:N_FIXED]
...
def run_1d_experiment(times: List[int], uniform_times: List[int], query_range: List[int]):
    ...
    nyc_max = max(times)
    range_end = nyc_max + 1
    timespan = nyc_max - nyc_min
    normalized_timespan = timespan
    os.makedirs(BASE_OUTPUT_DIR, exist_ok=True)
    
    distinct_count = len(times)
    step = timespan / (distinct_count - 1) if distinct_count > 1 else 1
    
    # Build 1D-Trees
    print("Building 1D-Tree for NYC Taxi times...")
    nyc_tree = build_1dtree(times, nyc_min, range_end)
    if nyc_tree is None:
        print("Failed to build NYC Taxi tree. Skipping.")
        return
    print("NYC Taxi 1D-Tree built.")
    
    print("Building 1D-Tree for Uniform times...")
    # === IMPORTANT FIX: offset uniform like Gowalla does ===
    uniform_times_offset = [nyc_min + t for t in uniform_times]
    uniform_tree = build_1dtree(uniform_times_offset, nyc_min, range_end)
    ...
    # Build 1D-DAG3
    print("Building 1D-DAG3 for NYC Taxi times...")
    nyc_dag3 = build_1ddag3(times, nyc_min, range_end)
    if nyc_dag3 is None: return
    print("NYC Taxi 1D-DAG3 built.")
    
    print("Building 1D-DAG3 for Uniform times...")
    uniform_dag3 = build_1ddag3(uniform_times_offset, nyc_min, range_end)
    if uniform_dag3 is None: return
    print("Uniform 1D-DAG3 built.")

    # Build 1D-DAG5
    print("Building 1D-DAG5 for NYC Taxi times...")
    nyc_dag5 = build_1ddag5(times, nyc_min, range_end)
    if nyc_dag5 is None: return
    print("NYC Taxi 1D-DAG5 built.")
    
    print("Building 1D-DAG5 for Uniform times...")
    uniform_dag5 = build_1ddag5(uniform_times_offset, nyc_min, range_end)
    if uniform_dag5 is None: return
    print("Uniform 1D-DAG5 built.")


    # Experiment loop - per query_range independent
    records_nyc_tree = {s: [] for s in query_range}
    records_uniform_tree = {s: [] for s in query_range}
    records_nyc_dag3 = {s: [] for s in query_range}
    records_uniform_dag3 = {s: [] for s in query_range}
    records_nyc_dag5 = {s: [] for s in query_range}
    ...
    for s in query_range:
        ...
        seed_offset = 0
        # Stabilization state
        nyc_dag3_stable = False
        nyc_dag5_stable = False
        u_dag3_stable = False
        u_dag5_stable = False
        nyc_dag3_stable_count = None
        nyc_dag5_stable_count = None
        u_dag3_stable_count = None
        u_dag5_stable_count = None
        # Empirical distribution trackers
        nyc_prev_dist_dag3 = None
        uniform_prev_dist_dag3 = None
        nyc_prev_dist_dag5 = None
        uniform_prev_dist_dag5 = None  
        nyc_penult_dist_dag3 = None
        uniform_penult_dist_dag3 = None
        nyc_penult_dist_dag5 = None
        uniform_penult_dist_dag5 = None

        while queries_count < MAX_QUERIES_PER_SIZE and not (nyc_dag3_stable and nyc_dag5_stable and u_dag3_stable and u_dag5_stable):
            ...
            for q_start, q_end in qs:
                # TREE
                res_nyc_tree = src_search_tree(nyc_tree, q_start, q_end)
                res_u_tree = src_search_tree(uniform_tree, q_start, q_end)
                records_nyc_tree[s].append((q_start, q_end, res_nyc_tree.level if res_nyc_tree else None))
                records_uniform_tree[s].append((q_start, q_end, res_u_tree.level if res_u_tree else None))
                # DAG3
                res_nyc_dag3 = src_search_dag3(nyc_dag3, q_start, q_end)
                res_u_dag3 = src_search_dag3(uniform_dag3, q_start, q_end)
                records_nyc_dag3[s].append((q_start, q_end, res_nyc_dag3.level if res_nyc_dag3 else None))
                records_uniform_dag3[s].append((q_start, q_end, res_u_dag3.level if res_u_dag3 else None))
                # DAG5
                res_nyc_dag5 = src_search_dag5(nyc_dag5, q_start, q_end)
                res_u_dag5 = src_search_dag5(uniform_dag5, q_start, q_end)
                records_nyc_dag5[s].append((q_start, q_end, res_nyc_dag5.level if res_nyc_dag5 else None))
            ...
            # Tree
            process_results(records_nyc_tree, s, tmp_counts, tree_output_dir, "nyc_tree", "blue")
            process_results(records_uniform_tree, s, tmp_counts, tree_output_dir, "uniform_tree", "green")
            # DAG3
            process_results(records_nyc_dag3, s, tmp_counts, dag3_output_dir, "nyc_dag3", "blue")
            process_results(records_uniform_dag3, s, tmp_counts, dag3_output_dir, "uniform_dag3", "green")
            # DAG5
            process_results(records_nyc_dag5, s, tmp_counts, dag5_output_dir, "nyc_dag5", "blue")
            process_results(records_uniform_dag5, s, tmp_counts, dag5_output_dir, "uniform_dag5", "green")

            # Compute current LDD dists
            nyc_current_dist_dag3 = compute_ldd(records_nyc_tree[s], records_nyc_dag3[s])
            uniform_current_dist_dag3 = compute_ldd(records_uniform_tree[s], records_uniform_dag3[s])
            nyc_current_dist_dag5 = compute_ldd(records_nyc_tree[s], records_nyc_dag5[s])
            uniform_current_dist_dag5 = compute_ldd(records_uniform_tree[s], records_uniform_dag5[s])


            # For 3-DAG (tree vs 3-DAG)
            l2_nyc_dag3 = float("nan")
            l2_u_dag3 = float("nan")
            if queries_count > INITIAL_QUERIES_PER_SIZE:
                if nyc_prev_dist_dag3 is not None:
                    prev_dict_nyc_dag3 = dict(zip(nyc_prev_dist_dag3[0], nyc_prev_dist_dag3[2]))
                    curr_dict_nyc_dag3 = dict(zip(nyc_current_dist_dag3[0], nyc_current_dist_dag3[2]))
                    l2_nyc_dag3 = compute_l2_norm(prev_dict_nyc_dag3, curr_dict_nyc_dag3)
            ...
            l2_nyc_dag5 = float("nan")
            l2_u_dag5 = float("nan")
            if queries_count > INITIAL_QUERIES_PER_SIZE:
                if nyc_prev_dist_dag5 is not None:
                    prev_dict_nyc_dag5 = dict(zip(nyc_prev_dist_dag5[0], nyc_prev_dist_dag5[2]))
                    curr_dict_nyc_dag5 = dict(zip(nyc_current_dist_dag5[0], nyc_current_dist_dag5[2]))
                    l2_nyc_dag5 = compute_l2_norm(prev_dict_nyc_dag5, curr_dict_nyc_dag5)
            ...
            nyc_penult_dist_dag3 = nyc_prev_dist_dag3 if nyc_prev_dist_dag3 else None
            uniform_penult_dist_dag3 = uniform_prev_dist_dag3 if uniform_prev_dist_dag3 else None
            nyc_penult_dist_dag5 = nyc_prev_dist_dag5 if nyc_prev_dist_dag5 else None
            uniform_penult_dist_dag5 = uniform_prev_dist_dag5 if uniform_prev_dist_dag5 else None

            if queries_count > INITIAL_QUERIES_PER_SIZE:
                if not nyc_dag3_stable and nyc_prev_dist_dag3 is not None and l2_nyc_dag3 < EPSILON:
                    nyc_dag3_stable = True
                    nyc_dag3_stable_count = queries_count
                    print(f"NYC DAG3 hit stable at {queries_count} for s={s}, L2: {l2_nyc_dag3:.6f}")
                    save_final_ldd(*nyc_current_dist_dag3, s, queries_count, "nyc_dag3", 
                                   l2_nyc_dag3, s_star=s_star, suffix="_stable")

                if not nyc_dag5_stable and nyc_prev_dist_dag5 is not None and l2_nyc_dag5 < EPSILON:
                    nyc_dag5_stable = True
                    nyc_dag5_stable_count = queries_count
                    print(f"NYC DAG5 hit stable at {queries_count} for s={s}, L2: {l2_nyc_dag5:.6f}")
                    save_final_ldd(*nyc_current_dist_dag5, s, queries_count, "nyc_dag5", 
                                   l2_nyc_dag5, s_star=s_star, suffix="_stable")
                ...
                if nyc_dag3_stable and nyc_dag5_stable and u_dag3_stable and u_dag5_stable:
                    print(f" All paths locked in at {queries_count} for s={s}")
                    save_final_ldd(*nyc_current_dist_dag3, s, queries_count, "nyc_dag3", 
                                   l2_nyc_dag3, s_star=s_star, suffix="_all_stable")
                    save_final_ldd(*nyc_current_dist_dag5, s, queries_count, "nyc_dag5", 
                                   l2_nyc_dag5, s_star=s_star, suffix="_all_stable")
                    save_final_ldd(*uniform_current_dist_dag3, s, queries_count, "uniform_dag3", 
            ...
            nyc_prev_dist_dag3 = nyc_current_dist_dag3
            uniform_prev_dist_dag3 = uniform_current_dist_dag3
            nyc_prev_dist_dag5 = nyc_current_dist_dag5
        ...
        if nyc_dag3_stable and nyc_dag5_stable and u_dag3_stable and u_dag5_stable:
          ...
          while extra_queries_count < extra_queries_target and queries_count < MAX_QUERIES_PER_SIZE:
              ...
              tmp_counts = {s: queries_count}
              # Tree
              process_results(records_nyc_tree, s, tmp_counts, tree_output_dir, "nyc_tree_extra", "blue")
              process_results(records_uniform_tree, s, tmp_counts, tree_output_dir, "uniform_tree_extra", "green")
              # DAG3
              process_results(records_nyc_dag3, s, tmp_counts, dag3_output_dir, "nyc_dag3_extra", "blue")
              process_results(records_uniform_dag3, s, tmp_counts, dag3_output_dir, "uniform_dag3_extra", "green")
              # DAG5
              process_results(records_nyc_dag5, s, tmp_counts, dag5_output_dir, "nyc_dag5_extra", "blue")
              ...
              save_final_ldd(nyc_current_dist_dag3[0], nyc_current_dist_dag3[1], nyc_current_dist_dag3[2], s, queries_count, "nyc_dag3", l2_nyc_dag3, s_star=s_star, suffix="_extra")
              ...
              save_final_ldd(nyc_current_dist_dag5[0], nyc_current_dist_dag5[1], nyc_current_dist_dag5[2], s, queries_count, "nyc_dag5", l2_nyc_dag5, s_star=s_star, suffix="_extra")
        ...
        nyc_final_dist_dag3 = compute_ldd(records_nyc_tree[s], records_nyc_dag3[s])
        uniform_final_dist_dag3 = compute_ldd(records_uniform_tree[s], records_uniform_dag3[s])
        nyc_final_dist_dag5 = compute_ldd(records_nyc_tree[s], records_nyc_dag5[s])
        ...
        if nyc_penult_dist_dag3 is not None:
            penult_dict_nyc_dag3 = dict(zip(nyc_penult_dist_dag3[0], nyc_penult_dist_dag3[2]))
            final_dict_nyc_dag3 = dict(zip(nyc_final_dist_dag3[0], nyc_final_dist_dag3[2]))
            l2_nyc_final_dag3 = compute_l2_norm(penult_dict_nyc_dag3, final_dict_nyc_dag3)
        else:
            l2_nyc_final_dag3 = float("nan")
        ...
        if nyc_penult_dist_dag5 is not None:
            penult_dict_nyc_dag5 = dict(zip(nyc_penult_dist_dag5[0], nyc_penult_dist_dag5[2]))
            final_dict_nyc_dag5 = dict(zip(nyc_final_dist_dag5[0], nyc_final_dist_dag5[2]))
            l2_nyc_final_dag5 = compute_l2_norm(penult_dict_nyc_dag5, final_dict_nyc_dag5)
        else:
            l2_nyc_final_dag5 = float("nan")
        ...
        save_final_ldd(nyc_final_dist_dag3[0], nyc_final_dist_dag3[1], nyc_final_dist_dag3[2], s, queries_count, "nyc_dag3", l2_nyc_final_dag3, s_star=s_star, suffix="_final")
        ...
        save_final_ldd(nyc_final_dist_dag5[0], nyc_final_dist_dag5[1], nyc_final_dist_dag5[2], s, queries_count, "nyc_dag5", l2_nyc_final_dag5, s_star=s_star, suffix="_final")
        ...
        if not (nyc_dag3_stable and nyc_dag5_stable):
            print(f"NYC Taxi did not fully stabilize (DAG3: {nyc_dag3_stable}, DAG5: {nyc_dag5_stable}) for s={s}; using {queries_count} queries")
...
def save_final_ldd(unique_k: np.ndarray, counts_k: np.ndarray, emp_probs: np.ndarray,
    ...
    L2_emp_theo = search_result['min_L2']
    ...
    l2_results.append({
        ...
        'max_k': max_k,
        'L2_emp': l2_emp,
        'L2_emp_theo': L2_emp_theo,
        'expected_exp_k_emp': expected_exp_k_emp,
        'expected_k_emp': expected_k_emp,
        'expected_k_theo': best_E_k,
        'expected_exp_k_theo': best_E_2k,
        'suffix': suffix,
        's_star': best_s_star
    })
...
if __name__ == "__main__":
    ...
    times = load_nyc_taxi_data(NYC_FILE)
    ...
    if final_table_rows:
        ...
        final_df.to_excel(os.path.join(BASE_OUTPUT_DIR, f"seed{SEED}_yellowtaxi_best_sstar_table.xlsx"), index=False)
        print(f"Saved final table: seed{SEED}_yellowtaxi_best_sstar_table.xlsx")
    ...
    # 3-DAG (blue) vs 5-DAG (red) side-by-side - NYC Taxi
    # ===================================================================
    plt.figure(figsize=(10, 6))

    query_ranges = [60, 3600, 86400, 604800]
    x_positions = np.arange(len(query_ranges))
    width = 0.35
    ...
    for s in query_ranges:
        # 3-DAG (blue)
        ds_df3 = l2_df[l2_df['dataset'] == 'nyc_dag3']
        ...
        # 5-DAG (red)
        ds_df5 = l2_df[l2_df['dataset'] == 'nyc_dag5']
    ...
    plt.title("NYC Taxi: 3-DAG vs 5-DAG (Expected Level Difference)", fontsize=15)
    plt.legend()
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_OUTPUT_DIR, "nyc_taxi_combined_level_diff_whisker.png"), 
                dpi=300, bbox_inches='tight')
    plt.close()

    # ===================================================================
    # COMBINED WHISKER PLOT: Expected FP-Competitive Ratio 
    # 3-DAG (blue) vs 5-DAG (red) side-by-side - NYC Taxi
    # ===================================================================
    plt.figure(figsize=(10, 6))
    ...
    plt.title("NYC Taxi: 3-DAG vs 5-DAG (Expected FP-Competitive Ratio)", fontsize=15)
    plt.legend()
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(BASE_OUTPUT_DIR, "nyc_taxi_combined_exp_ratio_whisker.png"), 
                dpi=300, bbox_inches='tight')
    plt.close()
